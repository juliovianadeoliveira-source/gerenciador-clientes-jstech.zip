...
function toClient(row){const stored=parseStoredNotes(row.notes,row);return{id:row.id,name:row.name,phone:row.phone,appName:row.app_name,startDate:row.start_date,plan:row.plan,value:Number(row.amount),payment:row.payment_method,notes:stored.notes,resellerId:row.reseller_id||null,resellerName:stored.resellerName||'',resellerPhone:stored.resellerPhone||'',cost:Number(row.cost??stored.cost)||0,devices:stored.devices.length?stored.devices:[{device:'TV 1',appName:row.app_name||'',mac:'',key:'',screens:Number(row.screens)||1}],dueDate:row.due_date,cancelled:row.cancelled,lifetime:!!row.lifetime,screens:Number(row.screens)||1,createdAt:row.created_at}}
function fromClient(data){return{name:data.name,phone:data.phone,app_name:(data.devices&&data.devices[0]?data.devices[0].appName:data.appName)||'',start_date:data.startDate,plan:data.plan,amount:data.value,cost:Number(data.cost)||0,reseller_id:data.resellerId||null,payment_method:data.payment,notes:DEVICE_MARKER+JSON.stringify({notes:data.notes||'',devices:data.devices||[],resellerName:data.resellerName||'',resellerPhone:data.resellerPhone||'',cost:Number(data.cost)||0}),due_date:data.dueDate,cancelled:!!data.cancelled,lifetime:!!data.lifetime,screens:Number(data.screens)||1}}
...
async function loadClients(){
  const [rows,resellers,apps]=await Promise.all([
    api('/rest/v1/clients?select=*&order=due_date.asc'),
    api('/rest/v1/resellers?select=id,name,phone&active=eq.true&order=name.asc'),
    api('/rest/v1/client_apps?select=*&active=eq.true&order=created_at.asc')
  ]);
  const resellerMap=new Map(resellers.map(r=>[r.id,r])),appsByClient=new Map();
  apps.forEach(a=>{const list=appsByClient.get(a.client_id)||[];list.push({id:a.id,device:a.device_name,appName:a.app_name,mac:a.mac||'',key:a.app_key||'',screens:Number(a.screens)||1,startDate:a.start_date,dueDate:a.due_date,lifetime:!!a.lifetime});appsByClient.set(a.client_id,list)});
  clients=rows.map(row=>{const c=toClient(row),reseller=resellerMap.get(row.reseller_id),devices=appsByClient.get(row.id);if(reseller){c.resellerId=reseller.id;c.resellerName=reseller.name;c.resellerPhone=reseller.phone||''}if(devices?.length)c.devices=devices;return c});
  render();
}
async function ensureReseller(name,phone){
  name=(name||'').trim();if(!name)return null;
  const rows=await api('/rest/v1/resellers?on_conflict=owner_id,name',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({name,phone:(phone||'').trim(),active:true})});
  return rows[0]?.id||null;
}
async function syncClientApps(clientId,data){
  await api('/rest/v1/client_apps?client_id=eq.'+encodeURIComponent(clientId),{method:'DELETE',headers:{Prefer:'return=minimal'}});
  const devices=(data.devices||[]).filter(d=>d.appName);
  if(!devices.length)return;
  await api('/rest/v1/client_apps',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(devices.map(d=>({client_id:clientId,device_name:d.device||'Dispositivo',app_name:d.appName,mac:d.mac||'',app_key:d.key||'',screens:Number(d.screens)||1,start_date:data.startDate,due_date:data.lifetime?null:data.dueDate,lifetime:!!data.lifetime,active:true})))});
}
