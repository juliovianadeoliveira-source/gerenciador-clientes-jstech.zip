...
function parseStoredNotes(value,row){
  try{
    if(typeof value==='string'&&value.indexOf(DEVICE_MARKER)===0){
      ...
      return {notes:parsed.notes||'',devices:Array.isArray(parsed.devices)?parsed.devices:[]};
    }
  }catch{}
  return {notes:value||'',devices:[{device:'TV 1',appName:row.app_name||'',mac:'',key:'',screens:Number(row.screens)||1}]};
}
function toClient(row){const stored=parseStoredNotes(row.notes,row);return{id:row.id,name:row.name,phone:row.phone,appName:row.app_name,startDate:row.start_date,plan:row.plan,value:Number(row.amount),payment:row.payment_method,notes:stored.notes,devices:stored.devices.length?stored.devices:[{device:'TV 1',appName:row.app_name||'',mac:'',key:'',screens:Number(row.screens)||1}],dueDate:row.due_date,cancelled:row.cancelled,lifetime:!!row.lifetime,screens:Number(row.screens)||1,createdAt:row.created_at}}
function fromClient(data){return{name:data.name,phone:data.phone,app_name:(data.devices&&data.devices[0]?data.devices[0].appName:data.appName)||'',start_date:data.startDate,plan:data.plan,amount:data.value,payment_method:data.payment,notes:DEVICE_MARKER+JSON.stringify({notes:data.notes||'',devices:data.devices||[]}),due_date:data.dueDate,cancelled:!!data.cancelled,lifetime:!!data.lifetime,screens:Number(data.screens)||1}}
...
function render(){renderSummary();let list=clients.filter(matchesFilter).filter(c=>`${c.name} ${c.phone} ${c.appName}`.toLowerCase().includes($('searchInput').value.toLowerCase().trim()));const sort=$('sortSelect').value;list.sort((a,b)=>sort==='name'?a.name.localeCompare(b.name,'pt-BR'):sort==='recent'?b.createdAt.localeCompare(a.createdAt):((a.dueDate||'9999-12-31').localeCompare(b.dueDate||'9999-12-31')));$('clientRows').innerHTML=list.map(clientRow).join('');$('emptyState').hidden=list.length!==0;document.querySelector('table').hidden=list.length===0;const titles={todos:'Todos os clientes',hoje:'Vencem hoje',amanha:'Vencem amanhã',proximos:'Próximos 5 dias',vencidos:'Clientes vencidos',ativos:'Clientes ativos',cancelados:'Clientes cancelados'};$('listTitle').textContent=titles[activeFilter];$('listSubtitle').textContent=`${list.length} cliente${list.length===1?'':'s'} encontrado${list.length===1?'':'s'}.`}
...
function openForm(client=null){renderDevices(client&&client.devices);$('clientForm').reset();$('clientId').value=client?.id||'';$('formTitle').textContent=client?'Editar cliente':'Novo cliente';$('startDate').value=client?.startDate||isoToday();$('plan').value=client?.plan||'30d';$('screens').value=String(client?.screens||1);$('lifetime').checked=!!client?.lifetime;toggleLifetimeFields();if(client){$('name').value=client.name;$('phone').value=client.phone;renderDevices(client.devices);$('value').value=client.value;$('payment').value=client.payment;$('notes').value=client.notes||''}updatePreview();$('clientDialog').showModal()}
